...
class CRNN(nn.Module):
    ...
    def encode(self, x):
        ...
        features = self.drop(
            F.relu(self.linear(x))
        )  # [bs,45,256]: 45x256 vector of image features
    ...
    def forward(self, images, targets=None):
        ...
        hiddens, _ = self.lstm(
            features
        )  # [1,45,256]: batch_size, sequence_length, hidden_dim

        if self.use_attention:
            attention, _ = self.attention(
                hiddens, features
            )  # torch.size([1,45,256]), 45 is the sequence length
            x = hiddens * attention
    ...
    def nll_loss(self, x, targets, sequence_length: int = 45):
        ...
        scalar = 20
        loss = (
            self.cross_entropy(x.view(-1, x.shape[-1]), targets.contiguous().view(-1))
            * scalar
        )
...
if __name__ == "__main__":
    ...
    model = CRNN(
        dims=256, num_chars=35, use_attention=True, use_ctc=True, grayscale=False
    )
